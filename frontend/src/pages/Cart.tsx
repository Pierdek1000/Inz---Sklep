...
export default function CartPage() {
  ...
  const inc = (id: string) => updateQty(id, (items.find(i => i.id === id)?.quantity || 1) + 1)
      ...
      {items.length === 0 ? (
        ...
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={{ xs: 2, md: 3 }} alignItems={{ xs: 'stretch', md: 'flex-start' }}>
                  ...
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', mt: 1, gap: 1 }}>
                      ...
                      <TextField
                        ...
                        size="small"
                      />
                      <IconButton aria-label="Zwiększ" onClick={() => inc(item.id)} size="small"><AddIcon /></IconButton>
                    </Box>
                  </Box>
          ...
          <Box sx={{ width: { xs: '100%', md: 360 }, borderRadius: 2, border: '1px solid', borderColor: 'divider', p: 2, position: 'sticky', top: 16 }}>
            ...
            <Button variant="contained" color="primary" fullWidth disabled={items.length === 0}>Przejdź do zamówienia</Button>
